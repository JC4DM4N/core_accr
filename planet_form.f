...
c     read in gas disc values for 500,000 years for disc from 0.0506Au to 19.854Au
...
c     sets random number interval [0.25, 12] for selecting rm values
      rm1 = 0.25d0
      rm2 = 15.0d0
      nok = 0
      nob = 0
...
c     fdust scales with star mass
c     maximum mass varies with solar mass
      maxfd = 2.492533*scaleM*scaleF*(Mstar/Mj)
c     minimum fdust is 0.1(Mstar/Msun)
...
c     a loop to set all planets as being initially in the simulation
         do 94 i=1,numpl
            check(i) = 1 !1=ok, 2=rh>disc, 3=mass limit exceeded
 94      continue

c     calculate fdust for the current disc
         fdust = minfd + fd*(l-1)
...
c     a loop to place the core at different radii in the disc from inner to outer disc rad
         do m = 1,numpl
            ...
            marker(m,1) = 0.0d0
            marker(m,2) = 0.0d0
c     Mass acreation tolerance adjusted according to radius
...
c     spaces out rad evenly from the inner radius
            do i = 2, numpts
               rad(i) = rad(i-1) + del_r
            enddo

c   sets the temperature at depending on radius
            do i = 1, numpts-1
...
c     this loop scales the gas surface density
            do i=1,1000
               do j=1,500
c                  SIGarray(i,j) = OSA(i,j)*fdust/maxfd*scaleF
c                  SIGarray(i,j) = OSA(i,j)*fdust*0.09242d0*(Msun/Mstar)
                   SIGarray(i,j) = OSA(i,j)
               enddo
            enddo

c     this loop calculates sigma gas based on a realistic model
            ...
            do j=1,500
               do while(RADarray(j).ge.rad(i))
                  if(i.eq.1)then !do the 1st point seperately (no 0 element)
                     ...
                     sigmagas(i) = SIGarray(1,j-1) - SIGfrac*(rad(i) - R
     $                    ADarray(j-1))
                  endif
...
c     this loop calculates sigmadust based on a simple model, then adds up the total mass
            iprint = 0
            do i = 1, numpts-1
               ...
               sigmadust(i) =
     &              fdust*eta_ice*10.0d0*(ro/rad(i))**(beta)
     ...
     *              (pi*rad(i+1)**2.0D0-pi*rad(i)**2.0D0)*sigmagas(i) +
     $              (pi*rad(i+1)**2.0D0-pi*rad(i)**2.0D0)*sigmadust(i)
               ...
               if (rad(i).lt.5.001*au .and. rad(i).gt.4.999*au .and.
     &             iprint.eq.0) then
     			  signorm = sigmadust(i)/sigma5au
                  print*, ''
                  print*, 'SIGMA AT 5AU, GAS:', sigmagas(i), 'DUST:',
     &                   sigmadust(i)/signorm, 'gcm-2'
                  print*, ''
                  iprint = 1
               endif
            ...
            initialMD(m) = 0.0
            do 2 i=1,numpts-1
               initialMD(m)=initialMD(m)+(sigmadust(i)*
     $              (pi*rad(i+1)**2-pi*rad(i)**2))
 2          continue

            totmass_old = totmass !keeps track of the origional mass

c     this loop sets "index" to be the number in the radius array where the planet is
            do i = 1, numpts-1
            ...
            Mpl = Mcore
            Mgiso = 50.0d0*(sigmagas(index)/2.4d3)**1.5d0*(rm/AU)**3.0*(
     $           Mstar/Msun)**(-0.5d0)
...
c     now that all the initial conditions have been set a time loop is used to itterate
c     over the lifetime of the disc
...
c     finds out the current array element for time
               do i=1,1000
                  Tindex = i
                  if(Tarray(i+1).gt.time*yr) exit
               enddo
c     calculates how far along to the next element the current time is
               Tfrac = time*yr - Tarray(Tindex)
               Tfrac = Tfrac/(Tarray(Tindex+1) - Tarray(Tindex))
c     now the sigmagas array is updated for this timestep
            i = 1               !start rad() off at 1
            do j=1,500
               do while(RADarray(j).ge.rad(i))
                  if(i.eq.1)then !do the 1st point seperately (no 0 element)
                     sigmagas(i) = (SIGarray(Tindex,j) - SIGarray(Tindex
     $                    +1,j))*Tfrac
                     ...
                     gasSig = SIGarray(Tindex+1,j-1) - SIGarray(Tindex+1
     $                    ,j)
...
c     checks the planet hasnt exceeded its isolation mass or mass limit
               If ((Mpl/Me.lt.Mgiso).and.(Mpl/Mj.lt.massLimit))
     &              Then
                  rhom = gasSig/H
                  rhocore = 3.2D0 ! Density of core
                  Rcore = (3.0D0*Mcore/(4.0D0*pi*rhocore))**
     &                                          (1.0D0/3.0D0)
                  Rc = Rcore
                  ...
                  RH = rm*
     *                 (Mpl/3.0D0/Mstar)**(1.0D0/3.0D0) !calculates the hill radius

                  call calcFg(Fg,Rc,RH,af,omega) !calculates the G enhancement

c     this sets the index1(inside) & index2(outside) values to the areas around the planets
c     area by calculating weather they are within its hill sphere
                  do j = 1, numpts-1
                     If ((rad(j).le.(rm-af)).and.(rad(j+1).gt.(rm-af)))
     &                    Then
                        index1 = j
                     EndIf
                     If ((rad(j).lt.(rm+af)).and.(rad(j+1).ge.(rm+af)))
     &                    Then
                        index2 = j+1
                     EndIf
                  enddo

c     condition to end simulation and discard planet for hill radius outwith disc
...
c     1st addition is from the inner edge of the hill sphere
                  adddust = 0.0d0
                  adddust =(pi*rad(index1+1)**2.0D0-
     &                 pi*(rm-af)**2.0D0)*sigmadust(index1)
                  Mdust = Mdust + adddust

c     this loop then covers all the areas within the planets hill sphere and adds
c     the dust mass in them to the value Mdust, the planets potential feeding mass
                  do j = index1+1, index2-2
                     adddust = 0.0d0
                     adddust =(pi*rad(j+1)**2.0D0-pi*rad(j)**2.0D0)
     *                    *sigmadust(j)
                     Mdust = Mdust + adddust
                  enddo

c     final contribution from outside edge of hill sphere
                  adddust = 0.0d0
                  adddust =(pi*(rm+af)**2.0D0-pi*rad(index2
     &                 -1)**2.0D0)*sigmadust(index2-1)
                  Mdust = Mdust + adddust
...
c     sets a temporart sigma over feeding zone to the average of outer & inner edges
                  sigmadust_temp = Mdust/(pi*(rm+af)**2.0D0
     *                 -pi*(rm-af)**2.0D0)
...
c     Equations to increase the core mass by the appropriate ammount
                  ...
                  Mcrit = 10.0d0*(Mdotcore*yr/Me/1.0d-6)  !critical mass for gas accreation
     *                 **(1.0d0/4.0d0)*kappa**(1.0d0/4.0d0)


c     gas is accreated if the mass exceeds the critical threshold
...
c     calculates the gas isolation mass corresponding to gap opening
                  Mgiso = 50.0d0*(sigmagas(index)/2.4d3)**1.5d0*(rm/AU)*
     $                 *3.0*(Mstar/Msun)**(-0.5d0) !includes decaying sigmagas

                  If (Mpl/Me.gt.Mgiso) then
                     Mdotgas = 0.0d0 !if isolation is achieved no gas accreated
                  EndIf

c     This bit reduces the time step so a smaller amount of gas is
c     added each run, so that Mgiso isn't greatly exceeded
...
c     recalculate temporary sigma for remaining dust
                  sigmadust_temp = (Mdust-dMcore)/(pi*(rm+af)**2.0D0
     *                 -pi*(rm-af)**2.0D0)
                  if(sigmadust_temp.le.0.0)then
                     sigmadust_temp = 0.0d0
                  endif

c     recalculates the inside edge sigmadust
...
c     all the sigmadust inside hill sphere of planet are set equal
                  do j = index1+1, index2-2
                     sigmadust(j) = sigmadust_temp
                  enddo

c     recalculates the outside edge sigmadust
...
c     the cores mass is altered dependint on total accreation
                  Mpl = Mpl + dMcore + dt*Mdotgas
                  AddUpGas(m) = AddUpGas(m) + dt*Mdotgas
                  AddUpEatenDust(m) = AddUpEatenDust(m) + dMcore

c                  print*, Mcore/Me, Mpl/Me, Mcrit, rm/AU

               end if !end mass limit $ Mgiso

               finalMD(m) = 0.0d0
               do 66 i=1,numpts-1
               finalMD(m)=finalMD(m)+(sigmadust(i)*((pi*rad(i+1))**2-(pi
     $                 *rad(i))**2))
 66            continue
...
c     recalculate conditions for new radius
...
**************** Type I migration ******************
     ...
     $                  /csm)**2.0d0*Omega*rm
                     ...
                     type1max30 = (2.7d0+1.1d0*betaMig)*
     &                   (50.0d0*Me/Mstar)*
     &                   (sigmagas(index)*(rm**2.0d0)/Mstar)*(rm*Omega
     $                    /csm)**2.0d0*Omega*rm
                     ...
                     if(randomMig.ne.0) then !code for turbulent type 1
                        ...
                        do while (type1time.lt.dt)
                           ...
                           Else
                             rm = rm - type1dir*type1max30*
     &                            type1dt -
     &                            (type1drbdt-type1max30)*
     &                            type1dt
                           EndIf
                           marker(m,1) = marker(m,1) + sqrt((type1drbdt*
     $                          type1dt)**2.0d0)
                           type1time = type1time + type1dt
                           if(type1time.gt.dt)then !corrects too much orbiting
                              rm = rm + type1dir*type1drbdt*(type1time-d
     $                             t)
...
c     exit if acreation virtually stopped
 ...
 17         continue !this continue is for a simulation exit due to too big RH
            ...
            finalcoreM(m)=Mcore
            finalpM(m)=Mpl
c     creates the final output, t,fd,fg,rad,Mgiso,Mplanet,Mcore,ocean(if snow line was crossed),
c     check(the status of planet),hill rad,feeding area,t1 magnitude, t2 magnitude
            write(19,*) time, fdust, fgas, rm/au, Mgiso,
     &           Mpl/Me, Mcore/Me, ocean, check(m), rH/Au, af/Au, marker
     $           (m,1)/Au, marker(m,2)/Au

               finalMD(m) = 0.0
               do 3 i=1,numpts-1
                  finalMD(m)=finalMD(m)+(sigmadust(i)*
     $                 (pi*rad(i+1)**2-pi*rad(i)**2))
 3             continue
...
c     a loop to count the number of planets exceeding the mass limit for each disc
         numstar = 0
         numlhr = 0
         numok = 0
         do 39 i=1,numpl
...
C
C     *******************************************************************************
C  Black box random number generator
C *******************************************************************************
C
      SUBROUTINE RANSET(ir,iseed)
C
C     This routine is used to initialize all of the
C     random number generators in RANPAK.
C
C     The seeding is performed as determined by ir
C     and iseed as follows:
C
C        ir <= 0, initialize all seeds
C        ir > 0, initialize only seed ir
C        iseed < 1, the default seed is used
C        iseed >= 1, use the value passed in iseed
C
C     After initialization, the value of the jth
C     random number seed should be accessed with
C     the RANGET routine.
C
C     CODE DEPENDENCIES: (internal RANPAK COMMON block)
C
C     DATE: DEC. 1, 1994
...
C
      ...
      IF (iseed.GT.0) THEN
         iset = iseed
      ELSE
C     Use the "demonic" default seed
         iset = 666
      ENDIF

      IF (ir.LT.1) THEN
C     Initialize all random number generator seeds
C     to iseed
         DO i=1,nrg
            isd(i) = iset
         ENDDO
...
C
C     PREFERRED DOUBLE PRECISION RANDOM NUMBER GENERATOR
C
C     The random number seed is set initially by a call to the
C     RANSET routine.  ir is a pointer to an internal random
C     number seed and NOT a random number seed.
C
C     COMMENTS: This routine will work correctly on any computer
C               with a maximum integer greater than or equal
C               to 2**31 - 1.  NOTE: The proper function of
C               the routine can verified by checking to see
C               that the random number seed after the generation
C               of 10000 random numbers is 1,043,618,065
C
C               For more information refer to the classic paper
C
C               Park, S.K. and Miller, K.W., "Random number generators:
C               good ones are hard to find."  Communications of the
C               ACM, Vol 31, No 10 (Oct. 1988).
C
C     CODE DEPENDENCIES: (internal RANPAK COMMON block)
C
C     DATE: DEC. 1, 1994
